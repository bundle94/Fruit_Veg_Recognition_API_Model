...
model = load_model('Fruits_Vegetable_Recognition_Model.h5')
...
def fetch_details(result):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("select * from public.foods where name = '"+result+"'")
    data = cur.fetchall()
    cur.close()
    conn.close()
    return data[0]
    ...
    if request.method == 'GET':
        ...
        if image.filename != '':
            if image and allowed_file(image.filename):
                ...
                details = fetch_details(result)
                res = {'name': result, 'category': fetch_category(result), 'calories': details[3], 'photo': details[4], 'nutrition_details': details[5]}
                #print(res)
                return jsonify(res)
        else:
